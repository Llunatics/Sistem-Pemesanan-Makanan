...
function load_cart() {
    ...
    if (cart !== null && cart.length > 0) {
        for (var i = 0; i < cart.length; i++) {
            ...
            item_price.innerHTML = "Rp. " + parseFloat(cart[i].price).toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2, minimumFractionDigits: 2 });
            total += cart[i].price;
        }
        total = Math.round(total * 100) / 100;
        localStorage.setItem('total_price', total);
        document.getElementById('total').innerHTML = "Rp. " + parseFloat(localStorage.getItem("total_price")).toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2, minimumFractionDigits: 2 });
